...
class Utility(object):
    ...
    def entropy(self, class_y):
        ...
        #############################################
        frequencies = np.bincount(class_y)/len(class_y)
    ...
    def information_gain(self, previous_y, current_y):
        ...
        # IG(Y, L, R) = E(Y) - (weight of L)* E(Y|L) - (weight of R)*E(Y|R)
    ...
    def best_split(self, X, y):
       ...
       'y_right': [0, 0, 1]
        ...
        for column in range(len(X[0])): # This iterator accounts for the number of columns available in X
            split_attribute = column
            column_values = sorted([x[column] for x in X])
            ...
            for val in column_values:
                ...
                if info_gain > current_best:
                    ...
                    best_split_info = {"best_split_feature": split_attribute,
...
class DecisionTree(object):
    ...
    def learn(self, X, y, par_node = {}, depth=0):
        ...
        '''
        --Stopping conditions--
        The stoppping conditions for the recursive function calls are:
            * max depth is reached
            * leaf node

        In this implementation, there are two types of leaf nodes: fail and output. And output leaf occurs when:
        -> All of y is same, i.e. entropy(y) = 0
        -> All of X is the same
        -> Max depth is reached

        A fail leaf occurs when:
        -> No input X or y is passed
        -> A node is unable to successfully split into two sub-nodes
        ...
        '''
        --Adding Sub-Trees to the Decision Tree--
        If the program has made it this far, then it is time to add nodes, or sub-trees, to our Decision Tree.
        '''
...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        ### Implement your code here
        #############################################
        random_indices = random.sample(range(len(XX)), n)

        for i in random_indices:
            sample.append(XX[i][:-1]) # Samples except last value, which is the label
            labels.append(XX[i][-1]) # Label
        #############################################
    ...
    def fitting(self):
        ...
        Iteratively train the the Decision Trees in this RF class.

        X: the bootstrapped datasets
        y: the bootstrapped labels

        '''
        for num in range(self.num_trees):
            self.decision_trees[num].learn(self.bootstraps_datasets[num],
                                           self.bootstraps_labels[num])
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                '''
                --Non-OOB Sample--
                Non-Out-of-Bag Samples are those that were not chosen for bootstrapping.This record must then
                be classified.
